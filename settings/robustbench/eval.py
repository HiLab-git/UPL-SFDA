...
from settings.robustbench.data import CORRUPTIONS, load_clean_dataset, \
    CORRUPTION_DATASET_LOADERS
from settings.robustbench.model_zoo.enums import BenchmarkDataset, ThreatModel
from settings.robustbench.utils import clean_accuracy, load_model, parse_args, update_json
from settings.robustbench.model_zoo import model_dicts as all_models
...
def benchmark(model: Union[nn.Module, Sequence[nn.Module]],
    ...
    #                                   batch_size=batch_size,
    #                                   device=device)
    if threat_model_ == ThreatModel.corruptions:
