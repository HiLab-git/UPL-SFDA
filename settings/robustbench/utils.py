...
from loss import DiceCeLoss,DiceLoss_n,DiceLoss
...
def clean_accuracy(model: nn.Module,
    ...
    with torch.no_grad():
        ...
        output = model(x_curr)
        # print(output.shape,'12121212')
        # segloss(output,y_curr)
        dice = DiceLoss(num_class)
        acc = dice(output, y_curr, True)
